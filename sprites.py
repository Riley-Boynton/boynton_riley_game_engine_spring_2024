...
class Enemy(pg.sprite.Sprite):
    def __init__(self, game, x, y):
        ...
        self.speed = 0
    def collide_with_walls(Enemy, dir):
        if dir == 'x':
            hits = pg.sprite.spritecollide(Enemy, Enemy.game.wals, False )
        if hits:
            if Enemy.vx > 0:
                Enemy.x = hits[0].rect.left - Enemy.rect.width
            if Enemy.vx < 0:
                Enemy.x = hits[0].rect.right
                Enemy.vx = 0
                Enemy.rect.x = Enemy.x
        if dir == 'y':
            hits = pg.sprite.spritecollide(Enemy, Enemy.game.walls, False )
            if hits:
                if Enemy.vy > 0:
                    Enemy.y = hits[0].rect.top - Enemy.rect.height
                if Enemy.vy < 0:
                    Enemy.y = hits[0].rect.bottom
                Enemy.vy = 0
